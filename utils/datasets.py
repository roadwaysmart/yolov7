...
class LoadStreamsEx:  # multiple IP or RTSP cameras, enhanced
    def __init__(self, sources='stream.txt', img_size=640, stride=32):
        self.mode = 'stream'
        self.img_size = img_size
        self.stride = stride
        self.queue = Queue()
        ...
        while cap.isOpened():
            ...
            if n == 4:  # read every 4th frame
                ...
                if success:
                    ...
                    self.queue.put((index, self.sources, img, img0, None))
                    self.count += 1
                    # print(f"{time.time():.3f}: image queued")
    ...
    def get(self):
        ...
        print(f"Image queued: {self.queue.qsize()}")
        return result
